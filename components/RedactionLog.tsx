          ...
          <tbody className="divide-y divide-gk-border">
            {entries.map((e, i) => (
              <tr key={i} className="text-gk-text">
                <td className="gk-mono px-3 py-2 text-gk-text-secondary">{e.field}</td>
                <td className="px-3 py-2">{e.category.replace(/_/g, " ")}</td>
                <td className="gk-mono px-3 py-2 text-gk-danger/90 line-through decoration-gk-danger/60">
                  {e.originalSnippet}
                </td>
                <td className="gk-mono px-3 py-2 text-gk-text-secondary">{e.replacement}</td>
